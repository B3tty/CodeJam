...
namespace IOError
{
    class Program
    {
        ...
        private static string RunCase(List<string> kase, int index)
        {
            ...
            var items = Enumerable.Range(0, c).Select(i =>
                kase[1].Substring(i * 8, 8)
            );
            ...
            {
                var binString = To10(str);
                var ascii = Convert.ToInt32(binString, 2);
                charList.Add(((char) ascii).ToString());
            }

            return $"Case #{index + 1}: {string.Join("", charList)}";
        }

        private static string To10(string str)
        {
            string result = "";
